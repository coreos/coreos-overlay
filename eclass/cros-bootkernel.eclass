...
# add a check to locate the cros-bootkernel/ regardless of what's going on.
ECLASSDIR_LOCAL=${BASH_SOURCE[0]%/*}
defconfig_dir() {
        local d="${ECLASSDIR}/cros-bootkernel"
        if [[ ! -d ${d} ]] ; then
                d="${ECLASSDIR_LOCAL}/cros-bootkernel"
        fi
        echo "${d}"
}
...
cros-bootkernel_src_prepare() {
	cros-workon_src_prepare
}

cros-bootkernel_src_configure() {

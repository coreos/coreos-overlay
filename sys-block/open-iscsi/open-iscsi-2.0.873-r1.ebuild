...
src_prepare() {
	...
	#rolling patch includes systemd support and correct socket names
	epatch "${FILESDIR}"/${P}-r1-rolling-20140514.patch
	epatch "${FILESDIR}"/${P}-r1-Makefiles.patch
    	touch ${S}/iscsiuio/NEWS
   	touch ${S}/iscsiuio/AUTHORS
}
...
src_install() {
	...
	dosbin utils/iscsi-iname utils/iscsi_discovery iscsiuio/src/unix/iscsiuio || die

	einfo "Installing configuration"
	insinto /lib/systemd/system
	doins etc/systemd/iscsid.service || die
	doins etc/systemd/iscsid.socket || die
}
